...
def random_padding(image: tf.Tensor, max_pad_w: int=3, max_pad_h: int=3) -> tf.Tensor:
    """
...
def data_loader(csv_filename: Union[List[str], str], params: Params, labels=True, batch_size: int=64,
    ...
    def input_fn():
        ...
        dataset = tf.contrib.data.CsvDataset(csv_filename, record_defaults=csv_types, header=False, field_delim=params.csv_delimiter, use_quote_delim=True)
        ...
        def _image_reading_preprocessing(path, width, height, label, xmin, ymin, xmax, ymax) -> dict():
            ...
            x_ratio = tf.random_uniform([],minval=-0.05,maxval=0.05,dtype=tf.float32,seed=None,name=None)
            y_ratio = tf.random_uniform([],minval=-0.05,maxval=0.05,dtype=tf.float32,seed=None,name=None)
            ...
            image = tf.image.crop_to_bounding_box(image, tf.cast(ymin, tf.int32), tf.cast(xmin, tf.int32), tf.cast(tf.subtract(ymax,ymin), tf.int32), tf.cast(tf.subtract(xmax,xmin), tf.int32)) 
            
            # 위에서 나온 값으로 image cropping
